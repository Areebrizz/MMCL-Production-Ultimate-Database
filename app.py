...
#     manpower_available integer default 0,
...
#     cycle_time_minutes float default 0,
...
#     passed_vehicles integer default 0,
...
def setup_god_admin():
    ...
    try:
        ...
        if not existing_admin:
            ...
            # Ensure the existing admin has the correct password if needed
            pass
    except Exception as e:
...
def calculate_manufacturing_metrics(records):
    ...
    # Calculate good_units from production_actual - scrap if good_units not available
    total_good_units = sum(record.get('production_actual', 0) - record.get('scrap', 0) for record in records)
...
def dashboard_page():
    ...
    with col3:
        ...
        try:
            users = get_all_users()
            dept_list = [u.get('department') for u in users if u.get('department')]
            departments.extend(list(set(dept_list)))
        except:
            pass
        ...
        if breakdown and isinstance(breakdown, dict):
            ...
            for cat, val in breakdown.items():
                if isinstance(val, (int, float)):
                    pareto_totals[cat] = pareto_totals.get(cat, 0) + val
                elif isinstance(val, dict):
                    # Handle nested dict structure if present
                    for sub_cat, sub_val in val.items():
                        pareto_totals[sub_cat] = pareto_totals.get(sub_cat, 0) + (sub_val or 0)
    ...
    with viz_tab2:
        if not df.empty:
            ...
            # Add good_units from production_actual - scrap
            scrap_by_date = df.groupby('date')['scrap'].sum().reset_index()
            production_data = production_data.merge(scrap_by_date, on='date', how='left')
            production_data['good_units'] = production_data['production_actual'] - production_data['scrap']
        ...
        for record in records:
            ...
            with st.expander(f"{record['date']} - {record['shift']} - {record['entered_by']} - {record['department']} - {status}"):
                ...
                with col2:
                    ...
                    st.write(f"**Downtime:** {record['downtime_hours']}h - {record.get('downtime_category', 'N/A')} ({record.get('downtime_reason', 'No reason')})")
